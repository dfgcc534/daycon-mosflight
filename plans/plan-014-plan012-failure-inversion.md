---
plan_id: 014
version: 4.3 (spec patch — plan-review-master iter 3 fix 5건. (1) §2.1.B.1 E2: K=9/13 anchor `±(a+b)/√2` 의 norm 0.01m 보장 가정 (dom/second/third 직교 unit-axis × 0.01m → ‖a+b‖=0.01√2, /√2 로 정확히 0.01m) 명시. K-Means 비직교 cluster 시 본 공식 미적용 박제. (2) §9.1 E4+E5 동시 채택 final loss formula 박제: `hybrid_combined_loss(use_reg_head=False, use_hinge=True)` 분기 = `loss = α·CE + 0.5·hit_aware_hinge(hybrid_pred, target)` (Huber 완전 제거, hinge 만). (3) §2.1.A.1 F0 1.98/1.20/−0.20 = dimensionless hard-coded constants (plan-006 grid-search carry, dt scaling 흡수) 명시 + v_last m/step / acc m/step² 단위 일관. (4) §5.4 G1 smoke val_hit_after 산출 path = `model.eval()` + `hybrid_predict()` (= inference + OOF 산출 path 와 동일) 명시. (5) §3.1 single-fold→5-fold sign mismatch sub-exp 제외 룰: anchor_5fold + best_lever_5fold 차이 < 0.005 시 `g5_no_additive` warn + anchor fallback (best_lever 자동 제외). v4.2 → v4.3.)
