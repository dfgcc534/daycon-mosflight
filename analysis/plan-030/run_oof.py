"""plan-030 run_oof — G1 (1-fold) + G3 (5-fold OOF) orchestrator.

Usage:
  python analysis/plan-030/run_oof.py --gate g1            # 1-fold (fold-0) only
  python analysis/plan-030/run_oof.py --gate g3            # full 5-fold OOF
  python analysis/plan-030/run_oof.py --gate g3 --quiet    # no per-epoch log
"""
...
_train_mod = _load(_THIS / "train.py", "p030_runoof_train")


def run_g1(verbose: bool = True) -> dict:
    """G1: fold-0 1-fold (PASS = hit_1cm > F0 - 0.003 = 0.6290)."""
    ...
    return {
        ...
        "PASS_threshold_hit_1cm": 0.6290,  # F0 0.6320 - 0.003 (G2 threshold)
        "PASS": hit_1cm > 0.6290,
    }


def run_g3(verbose: bool = True) -> dict:
    """G3: 5-fold concat OOF (PASS = hit_1cm >= 0.6360, STRONG >= 0.6387, EXCELLENT >= 0.65)."""
...
def _dump(result: dict, path: Path):
    """jsonable dump (numpy arrays separately)."""
    arrays = {}
...
def main():
    ...
    summary = {k: v for k, v in result.items() if k not in ("oof_pred", "oof_probs", "fold_logs", "gt_anchor_label")}
